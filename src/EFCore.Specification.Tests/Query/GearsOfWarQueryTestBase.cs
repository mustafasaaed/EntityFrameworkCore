...
#pragma warning restore IDE0031 // Use null propagation
        ...
        public virtual void Correlated_collections_left_join_with_self_reference()
        {
            AssertQuery<CogTag, Gear>(
                (ts, gs) =>
                    ...
                    select new { t.Note, ReportNames = o.Reports.Select(r => r.FullName) },
                (ts, gs) =>
                    from t in ts
                    join o in gs.OfType<Officer>() on t.GearNickName equals o.Nickname into grouping
                    from o in grouping.DefaultIfEmpty()
        ...
        public virtual void Correlated_collections_deeply_nested_left_join()
        {
            AssertQuery<CogTag, Gear>(
                (ts, gs) =>
                    ...
                    select g.Squad.Members.Where(m => m.HasSoulPatch).Select(m => new { m.Nickname, AutomaticWeapons = m.Weapons.Where(w => w.IsAutomatic) }),
                (ts, gs) =>
                    from t in ts
                    join g in gs on t.GearNickName equals g.Nickname into grouping
                    from g in grouping.DefaultIfEmpty()
        ...
        {
            AssertQuery<Weapon>(
                ws => ws.OrderBy(w => w.Name).Select(w => w.Owner.Squad.Members.OrderByDescending(m => m.FullName).Select(m => new { Weapons = m.Weapons.Where(ww => !ww.IsAutomatic).OrderBy(ww => ww.Id), m.Rank })),
